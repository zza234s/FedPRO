...
class clientCP(Client):
    ...
    def save_best_model(self,best_acc_str):
        ...
        context_path = os.path.join(self.save_folder_name, f"{self.role}_context_{best_acc_str}.pt")
        pm_test_path = os.path.join(self.save_folder_name, f"{self.role}_pm_test_{best_acc_str}.pt")

        torch.save(self.model.state_dict(), model_path)
        torch.save(self.context, context_path)
        torch.save(self.pm_test, pm_test_path)
        print(f"Save the FedCP's best local model files at: {self.save_folder_name}")
