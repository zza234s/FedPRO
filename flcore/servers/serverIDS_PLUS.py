...
import math
import numpy as np
from flcore.clients.clientIDS_PLUS import clientIDS_PLUS
...
#这个版本每个客户端在训练/推理时，用RAG的思路结合接收到的原型来增强样本的表征，随后将增强后的表征输入分类器中得到分类结果，但是效果不佳。
#尝试过的改进有：仅在推理时引入RAG思路增强；去掉prototype-based output，让RAG enhanced-output和local output集成后作为预测结果；




class FedIDS_PLUS(Server):
    def __init__(self, args, times):
        super().__init__(args, times)

        # select slow clients
        self.set_slow_clients()
        self.set_clients(clientIDS_PLUS)
