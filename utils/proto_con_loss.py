...
class ProtoConloss(nn.Module):
    ...
    def forward(self, features, labels, protos,proto_labels, num_classes):
        """Compute contrastive loss between feature and global prototype
        """
        device = features.device
        proto_mask = (labels.unsqueeze(1) == proto_labels.unsqueeze(0)).bool() # (batch_size, class_num)，用来标注每一个样本属于哪个类别
        '''
        ...
        contrast_feature = F.normalize(protos, p=2, dim=1)
        ...
        # compute log_prob
        exp_logits = torch.exp(logits) #* (~proto_mask) #L_g和L_p中分式的分母，~onehot_mask中每一个样本所属类对应的元素为0，其他为1
        log_prob = logits - torch.log(exp_logits.sum(1, keepdim=True))

        # compute mean of log-likelihood over positive
        mean_log_prob_pos = (proto_mask * log_prob).sum(1)
